(function(){
 ...
 let registros=[],naoPendenciamentos=[],carregando=false,chamadoAtual='';
 ...
 async function carregar(){
  ...
  try{const [{data:rows,error},{data:negativas,error:negErro}]=await Promise.all([SB.from('solicitacoes_pendencia').select('*').order('solicitado_em',{ascending:false}).limit(500),SB.from('nao_pendenciamentos_chamados').select('*').order('criado_em',{ascending:false}).limit(500)]);if(error)throw error;if(negErro)throw negErro;registros=rows||[];naoPendenciamentos=negativas||[];renderAdmin();if(SB_PROFILE.tipo==='tecnico'&&typeof rTat==='function')rTat();}catch(err){console.warn('Pendências:',err.message);}finally{carregando=false;}
 }
 function resumoTecnico(id){const r=ultima(id),n=naoPendenciamentos.filter(x=>x.chamado_id===id).sort((a,b)=>String(b.criado_em).localeCompare(String(a.criado_em)))[0];if(n&&(!r||String(n.criado_em)>String(r.solicitado_em)))return `<div class="info" style="margin-top:8px;border-left:4px solid var(--r)"><b>Chamado não pendenciado pelo operador</b><br><b>Motivo:</b> ${safe(n.motivo)}<br>${safe(n.operador_nome)} • ${safe(data(n.criado_em))}</div>`;if(!r)return '';const cor=r.status==='aprovada'?'var(--g)':r.status==='rejeitada'?'var(--r)':'var(--y)';return `<div class="info" style="margin-top:8px;border-left:4px solid ${cor}"><b>${safe(estados[r.status]||r.status)}</b><br>${safe(motivos[r.motivo]||r.motivo)} — ${safe(r.mensagem)}<br>Novo prazo: ${safe(data(r.aprazado_para))} • Saída programada: ${safe(data(r.saida_programada_em))}${r.decisao_mensagem?`<br><b>Retorno do operador:</b> ${safe(r.decisao_mensagem)}`:''}</div>`;}
 function renderAdmin(){
  const box=e('lista-pendencias-admin');if(!box||typeof SB_PROFILE==='undefined'||SB_PROFILE?.tipo!=='admin')return;
  const solicitacoes=registros.map(r=>{const c=typeof D!=='undefined'?D.ch.find(x=>x.id===r.chamado_id):null;return `<div class="os"><div class="os-h"><span class="os-id">${safe(c?.pr||'Chamado')}</span><span class="badge ${r.status==='aprovada'?'b-concluida':r.status==='rejeitada'?'b-recusado':'b-pendente'}">${safe(estados[r.status]||r.status)}</span></div><div class="os-r"><b>${r.origem==='tecnico'?'Solicitação recebida do técnico':'Solicitado pelo operador'}:</b> ${safe(r.solicitado_por_nome)}</div><div class="os-r"><b>Situação informada:</b> ${safe(motivos[r.motivo]||r.motivo)}</div><div class="info" style="margin:8px 0"><b>Mensagem recebida:</b><br>${safe(r.mensagem)}</div><div class="os-r"><b>Novo prazo:</b> ${safe(data(r.aprazado_para))} • <b>Saída programada:</b> ${safe(data(r.saida_programada_em))}</div>${r.decidido_por_nome?`<div class="os-r"><b>Decisão do operador:</b> ${safe(r.decidido_por_nome)} em ${safe(data(r.decidido_em))}${r.decisao_mensagem?`<br><b>Motivo da decisão:</b> ${safe(r.decisao_mensagem)}`:''}</div>`:''}</div>`;});const negativas=naoPendenciamentos.map(n=>{const c=typeof D!=='undefined'?D.ch.find(x=>x.id===n.chamado_id):null;return `<div class="os"><div class="os-h"><span class="os-id">${safe(c?.pr||'Chamado')}</span><span class="badge b-recusado">Não pendenciado</span></div><div class="os-r"><b>Operador:</b> ${safe(n.operador_nome)}</div><div class="info" style="margin:8px 0"><b>Motivo do não pendenciamento:</b><br>${safe(n.motivo)}</div><div class="os-r"><b>Registrado em:</b> ${safe(data(n.criado_em))}</div></div>`;});box.innerHTML=[...solicitacoes,...negativas].join('')||'<div class="empty">Nenhuma solicitação ou decisão de pendência registrada.</div>';
 }
 ...
 if(typeof SB!=='undefined')SB.channel('pendencias-internas').on('postgres_changes',{event:'*',schema:'public',table:'solicitacoes_pendencia'},carregar).on('postgres_changes',{event:'*',schema:'public',table:'nao_pendenciamentos_chamados'},carregar).on('postgres_changes',{event:'UPDATE',schema:'public',table:'chamados'},carregar).subscribe();
