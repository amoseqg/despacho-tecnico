(function(){
  ...
  function renderPendencia(c){
    ...
    const detalhe=p?`<div class="pendencia-resumo ${p.status}"><div class="item-topo"><strong>Pendência</strong><span class="badge ${p.status==='aprovada'?'validado':p.status==='rejeitada'?'nao-validado':'aguardando'}">${p.status==='solicitada'?'Aguardando decisão':p.status==='aprovada'?'Autorizada':p.status==='rejeitada'?'Não autorizada':'Finalizada'}</span></div><p><b>Solicitado por:</b> ${escapar(p.solicitado_por_nome)} (${p.origem==='tecnico'?'Técnico':'Operador'})<br><b>Situação:</b> ${escapar(motivosPendencia[p.motivo]||p.motivo)}<br><b>Mensagem:</b> ${escapar(p.mensagem)}<br><b>Novo prazo:</b> ${escapar(dataLocal(p.aprazado_para))}<br><b>Saída programada:</b> ${escapar(dataLocal(p.saida_programada_em))}${p.decisao_mensagem?`<br><b>Retorno:</b> ${escapar(p.decisao_mensagem)}`:''}</p>${p.status==='solicitada'?`<div class="decisao-pendencia"><button type="button" class="botao botao-primario btn-decidir-pendencia" data-id="${p.id}" data-decisao="aprovada">Autorizar pendência</button><button type="button" class="botao botao-perigo btn-decidir-pendencia" data-id="${p.id}" data-decisao="rejeitada">Não autorizar</button></div>`:''}</div>`:'';
    ...
    return `<div class="pendencia-operador">${detalhe}${formulario}</div>`;
  }
  ...
  async function carregarChamados(){
    ...
    const ids=data.map(c=>c.id),[{data:execucoes,error:execErro},{data:validacoes,error:valErro},{data:exclusoes,error:excErro},{data:pendencias,error:penErro}]=await Promise.all([sb.from('execucoes').select('chamado_id,relatorio_encerramento,enviado_validacao_em').in('chamado_id',ids),sb.from('validacoes_encerramento').select('chamado_id,validacao,senha,resultado,motivo_nao_validacao,validado_em').in('chamado_id',ids),sb.from('exclusoes_chamados_portal').select('chamado_id,excluido_por_nome,excluido_por_email,excluido_em').in('chamado_id',ids),sb.from('solicitacoes_pendencia').select('*').in('chamado_id',ids).order('solicitado_em',{ascending:false})]);if(execErro)throw execErro;if(valErro)throw valErro;if(excErro)throw excErro;if(penErro)throw penErro;
    const exMap=new Map((execucoes||[]).map(e=>[e.chamado_id,e])),valMap=new Map((validacoes||[]).map(v=>[v.chamado_id,v])),excMap=new Map((exclusoes||[]).map(v=>[v.chamado_id,v])),penMap=new Map();(pendencias||[]).forEach(p=>{if(!penMap.has(p.chamado_id))penMap.set(p.chamado_id,p);});
    ...
    chamadosRelatorio=data.map(c=>({...c,execucao:exMap.get(c.id)||null,validacao:valMap.get(c.id)||null,exclusao:excMap.get(c.id)||null,pendencia:penMap.get(c.id)||null}));
  ...
  async function decidirPendencia(pendenciaId,decisao,botao){
    let mensagem=null;if(decisao==='rejeitada'){mensagem=texto(prompt('Informe o motivo da não autorização:')||'');if(mensagem.length<3){status($('chamado-status'),'Descreva o motivo da não autorização.');return;}}
    bloquear(botao,true,decisao==='aprovada'?'Autorizando...':'Recusando...');try{const {error}=await sb.rpc('decidir_pendencia_operador',{p_pendencia_id:pendenciaId,p_decisao:decisao,p_mensagem:mensagem});if(error)throw error;status($('chamado-status'),decisao==='aprovada'?'Pendência autorizada. O prazo está pausado até a saída programada.':'Pendência não autorizada; o técnico já pode visualizar o motivo.',true);await carregarChamados();}catch(e){status($('chamado-status'),'Não foi possível decidir a pendência: '+e.message);}finally{bloquear(botao,false,decisao==='aprovada'?'Autorizar pendência':'Não autorizar');}
  }
  ...
  $('lista-chamados').addEventListener('click',e=>{const validar=e.target.closest('.btn-validar-encerramento'),excluir=e.target.closest('.btn-excluir-chamado'),abrirPendencia=e.target.closest('.btn-abrir-pendencia'),enviarPendencia=e.target.closest('.btn-enviar-pendencia'),decidir=e.target.closest('.btn-decidir-pendencia');if(validar)validarEncerramentoOperador(validar.dataset.id,validar);if(excluir)excluirChamado(excluir.dataset.id,excluir);if(abrirPendencia)prepararFormularioPendencia(abrirPendencia.dataset.id);if(enviarPendencia)solicitarPendenciaOperador(enviarPendencia.dataset.id,enviarPendencia);if(decidir)decidirPendencia(decidir.dataset.id,decidir.dataset.decisao,decidir);});
